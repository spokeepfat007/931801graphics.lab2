...
class MainApp(QtWidgets.QMainWindow, design.Ui_MainWindow):
    ...
    def findImageBorders(self, matrix):
        ...
        minX, minY, maxX, maxY = self.findScreenBorders(matrix,[minX,minY,maxX, maxY])
    ...
    def findScreenBorders(self, matrix, imageBorders):
        ...
        minX, maxX = self.switch(value,PointsMaxX, PointsMinX, maxX,minX,self.width,2)
        value = [((maxY-minY)>self.height/2),(PointsMaxY-PointsMinY)>self.height/2,(self.height/2-(PointsMaxY-PointsMinY))/2<(maxY-PointsMaxY),(self.height/2-(PointsMaxY-PointsMinY))/2<(PointsMinY-minY)]
        minY, maxY = self.switch(value,PointsMaxY, PointsMinY, maxY,minY,self.height,1)
        return minX, minY, maxX, maxY
    ...
    def simpleTransformation(self, imageBorders, ObrMatrix, painter, image_data):
        ...
        for i in range(int(minX),int(maxX)):
            for j in range(int(minY), int(maxY)):
                ...
                painter.drawPoint(i+481-minX, j+1-minY)
                self.update()
        ...
        for i in range(int(minX),int(maxX),1):
            for j in range(int(minY), int(maxY),1):
                ...
                if ((z[0]<0) or (z[0]>(self.label.width()-1)) or (z[1]<0) or (z[1] > (self.label.height()-1))):
                    ...
                    # r, g, b = [round(i) for i in(colours)] #Целочисленное округление, если нужно
                    painter.setPen(QPen(QColor(r,g,b), 1, Qt.SolidLine))
                painter.drawPoint(i+481-minX, j+1-minY)
                self.update()
        ...
        for i in range(int(minX),int(maxX)):
            for j in range(int(minY), int(maxY)):
                ...
                if ((z[0]<0) or (z[0]>(self.label.width()-1)) or (z[1]<0) or (z[1] > (self.label.height()-1))):
                    ...
                    Im = (np.float_(matrixes[m][int(math.ceil(z[1])//pow(2,m)*math.ceil(self.label.width()/pow(2,m))+math.ceil(z[0])/pow(2,m))])*(z[0]-math.floor(z[0])) +
                             np.float_(matrixes[m][int(math.ceil(z[1])//pow(2,m)*math.ceil(self.label.width()/pow(2,m))+math.floor(z[0])/pow(2,m))])*(math.ceil(z[0])-z[0]))*(z[1]-math.floor(z[1])) + \
                            (np.float_(matrixes[m][int(math.floor(z[1])//pow(2,m)*math.ceil(self.label.width()/pow(2,m))+math.ceil(z[0])/pow(2,m))])*(z[0]-math.floor(z[0])) +
                             np.float_(matrixes[m][int(math.floor(z[1])//pow(2,m)*math.ceil(self.label.width()/pow(2,m))+math.floor(z[0])/pow(2,m))])*(math.ceil(z[0])-z[0]))*(math.ceil(z[1])-z[1])
                    #Im=matrixes[m][int(z[1]/pow(2,m))*math.ceil(self.label.width()/pow(2,m))+int(z[0]/pow(2,m))]
                    I2m = (np.float_(matrixes[m+1][int(math.ceil(z[1])//(2*pow(2,m))*math.ceil(self.label.width()/(2*pow(2,m)))+math.ceil(z[0])/(2*pow(2,m)))])*(z[0]-math.floor(z[0])) +
                          np.float_(matrixes[m+1][int(math.ceil(z[1])//(2*pow(2,m))*math.ceil(self.label.width()/(2*pow(2,m)))+math.floor(z[0])/(2*pow(2,m)))])*(math.ceil(z[0])-z[0]))*(z[1]-math.floor(z[1])) + \
                         (np.float_(matrixes[m+1][int(math.floor(z[1])//(2*pow(2,m))*math.ceil(self.label.width()/(2*pow(2,m)))+math.ceil(z[0])/(2*pow(2,m)))])*(z[0]-math.floor(z[0])) +
                          np.float_(matrixes[m+1][int(math.floor(z[1])//(2*pow(2,m))*math.ceil(self.label.width()/(2*pow(2,m)))+math.floor(z[0])/(2*pow(2,m)))])*(math.ceil(z[0])-z[0]))*(math.ceil(z[1])-z[1])
                    #I2m=matrixes[2*m][int(int(z[1]/(2*pow(2,m)))*math.ceil(self.label.width()/(2*pow(2,m)))+int(z[0])/(2*pow(2,m)))]
                    color=[]
                    for rad in range(0,3):
                        color+=[(Im[rad]*((2*pow(2,m)-K)/pow(2,m))+I2m[rad]*(K-pow(2,m)))/pow(2,m)]
    ...
    def nonlinearSolver(self):
        ...
        #self.a=[58, 346, 168, 71, 210, 355, 42, 142, 68, 90, 104, 145]
